  ...
  methods: {
    loginToCRM() {
      ...
      weiXinLogin({area: 2}).then((res) => {
        ...
        let redirect_uri = encodeURI('http://osc.chinabidding.cn/osc/qywechatlogin?flag=2');
        this.wx.ready(() => {
          this.wx.invoke(
            "openDefaultBrowser",
            {
              // url: url, // 在默认浏览器打开redirect_uri，并附加code参数；也可以直接指定要打开的url，此时不会附带上code参数。
              url: 'https://open.weixin.qq.com/connect/oauth2/authorize?appid=wwaf72994d2eb35329&redirect_uri=' + redirect_uri + '&response_type=code&scope=snsapi_base&state=1#wechat_redirect'
            },
            function (res) {
