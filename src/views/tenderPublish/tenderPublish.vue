    ...
    <Card>
      ...
      <Row>
        ...
        <template slot-scope="{ row }" slot="status">
            <Tag color="default" v-show="row.status === 2">待审核</Tag>
            <Tag color="red" v-show="row.status === 3">已打回</Tag>
            <Tag color="green" v-show="row.status === 4">已发布</Tag>
          </template>
          <template slot-scope="{ row }" slot="operation">
              <Row>
                <Button type="success" style="margin-right:10px;" :disabled='row.status == 4 || row.status == 3' @click='publish(row)'>发布</Button>
              <Button type="primary" :disabled='row.status == 3 || row.status == 4' @click='reject(row)'>打回</Button>
              </Row>
          </template>
        </Table>
      </Row>
...
<script>
import { getAutoBidList,autoBidPublish,autoBidReject } from "@/api/publish";
export default {
  name: "tenderPublish",
  data() {
    return {
  ...
  methods: {
    getDataList() {
      this.loading = true;
      getAutoBidList(this.searchData).then((res) => {
        if (res.success) {
    ...
    // 前往详情页面
    toDetail(v) {
      let data = this.getStore("autoDetailsTabs") || [];
      if (data.length !== 0) {
      ...
      this.setStore("currentTenderPublishId", v.id + "");
      this.$router.push('/tenderPublish/tenderDetail#' + v.id );
    },
    // 查询
    handleSearch() {
