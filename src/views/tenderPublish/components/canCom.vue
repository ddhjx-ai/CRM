  ...
  <div class="canCom">
    <div class="inner-side">
      ...
      >
        ...
        <Row class="rowBox">
          <div class="infoText">
            <div v-for="(item,index) in data.bidSectionInfoList" :key="index" style="margin-bottom:30px;">
              <p style="font-size:14px; font-weight:bold">标段（包）{{index + 1}} {{item.title}}：</p>
              ...
              <p style="margin:20px 0 10px 20px;">2、中标候选人按照招标文件要求承诺的项目负责人情况</p>
              <table
                cellspacing="0"
                class="bidderList"
              >
                ...
                <tr v-for="(manItem, manIndex) in item.bidderList" :key="manIndex">
                  <td >{{manIndex+1}}</td>
                  <td >{{manItem.candidateName}}{{manItem.unionCompanyName ? '，' + manItem.unionCompanyName : ''}}</td>
                  <td >{{manItem.projectManager}}</td>
                  <td >{{ manItem.certificate }}</td>
                </tr>
              </table>

              <p style="margin:20px 0 10px 20px;">3、中标候选人响应招标文件要求的资格能力条件</p>
              <table
                cellspacing="0"
                class="bidderList"
              >
                ...
                <tr v-for="(abItem, abIndex) in item.bidderList" :key="abIndex">
                  <td >{{abIndex + 1}}</td>
                  <td >{{abItem.candidateName}}{{abItem.unionCompanyName ? '，' + abItem.unionCompanyName : ''}}</td>
